...
print("\nsizing: one flat premium budget for every trade")
check("$7k at ask 1.00 -> $800 = 8", rb.size_qty(7_000, 1.00), 8)
check("$7k at ask 1.22 -> 6", rb.size_qty(7_000, 1.22), 6)
check("$7k at ask 0.70 (09-25 loser) -> 11, not 15", rb.size_qty(7_000, 0.70), 11)
check("$7k at ask 0.86 (09-25 winner) -> 9, not 5", rb.size_qty(7_000, 0.86), 9)
check("scales: $14k -> 16", rb.size_qty(14_000, 1.00), 16)
check("half size on data days", rb.size_qty(7_000, 1.00, 0.5), 4)
check("cheap contracts hit the cap", rb.size_qty(7_000, 0.10), rb.MAX_QTY)

print("\nbreakout across the clock gap (the 09-25 blind spot)")
check("SPY 768.40 -> 771.52 on a call = +3.12", round(rb.bridge_move("C", 771.52, 768.40), 2), 3.12)
check("that clears a 0.52 R", rb.bridge_move("C", 771.52, 768.40) >= 0.52, True)
check("against a put it is negative", rb.bridge_move("P", 771.52, 768.40) < 0, True)
check("no data -> 0", rb.bridge_move("C", None, 768.40), 0.0)
